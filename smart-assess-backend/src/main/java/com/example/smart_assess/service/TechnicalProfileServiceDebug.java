...
/**
 * Service de debug pour diagnostiquer les problèmes d'extraction de données
 * Utilisé temporairement pour identifier pourquoi les données d'analyse CV ne sont pas extraites correctement
 */
@Service
public class TechnicalProfileServiceDebug {
    
    private static final Logger log = LoggerFactory.getLogger(TechnicalProfileServiceDebug.class);
    
    /**
     * Méthode de debug pour diagnostiquer l'extraction des données du profil technique
     * Cette méthode sera appelée depuis TechnicalProfileService pour obtenir des logs détaillés
     */
